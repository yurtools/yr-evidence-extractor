...
// KEY REQUIREMENT IMPLEMENTED HERE:
// - Always output facts/claims/opinions text in ENGLISH.
// - Evidence must be an exact quote snippet from the original (any language).
...
- Evidence must be an EXACT QUOTE snippet from the provided text in the ORIGINAL LANGUAGE (do not translate evidence).
...
function parseModelJson(raw) {
    ...
    // ========= Prompt rendering (Python template -> string) =========
    function renderPromptFromPythonTemplate(pyTemplate, vars) {
        const t = (pyTemplate || "").trim();
        if (!t) pyTemplate = DEFAULT_PROMPT_PY;
    ...
    async function callOpenAICompatible({ url, apiKey, model, prompt, temperature = 0.2, useJsonSchema = false }) {
        ...
        // LM Studio requires response_format.type = json_schema or text
        if (useJsonSchema) {
    ...
    refreshBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            if (!extracted?.ok) throw new Error("Failed to extract text from page.");
